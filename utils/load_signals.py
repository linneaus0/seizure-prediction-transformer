        ...
        if data_type == 'ictal':
            ...
            if len(indices) > 0:
                ...
                for i in range(len(indices)):
                    st = szstart[indices[i]]*256 - 3 * 60 * 256  # SPH=3min
...
class PrepData():
    ...
    def preprocess(self, data_):
        ictal = self.type == 'ictal'
        interictal = self.type == 'interictal'
        targetFrequency = self.freq  # re-sample to target frequency,256hz
        numts = 30  # 28    # the length of window
            ...
            for data in mat_data:
                ...
                for i in range(totalSample):
                    if (i+1)*window_len <= data.shape[0]:
                        ...
                        stft_data = np.transpose(stft_data,(2,1,0))     # [Freq, Time, Channel] -> [Channel, Time, Freq]
                        stft_data = np.abs(stft_data)+1e-6
                        ...
                        if self.settings['dataset'] == 'FB':
                            stft_data = np.concatenate((stft_data[:,:,1:47],
                                                        ...
                                                        stft_data[:,:,104:]),
                                                       axis=-1)
                        elif self.settings['dataset'] == 'CHBMIT':  # filter like
