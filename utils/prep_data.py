...
def train_val_loo_split(ictal_X, ictal_y, interictal_X, interictal_y, val_ratio):
    ...
    for i in range(nfold):
        ...
        # process the data and label to 4-D and concat input along time axis
        y_train = np.repeat(y_train, X_train.shape[3])
        y_val = np.repeat(y_val, X_val.shape[3])
        y_test = np.repeat(y_test, X_test.shape[3])

        X_train = np.reshape(X_train, (-1, X_train.shape[2], X_train.shape[3], X_train.shape[4]))
        X_train = np.concatenate(np.transpose(X_train, (0, 2, 1, 3)), axis=0)   # [num, T, C, F] -> [T, C, F]
        X_val = np.reshape(X_val, (-1, X_val.shape[2], X_val.shape[3], X_val.shape[4]))
        X_val = np.concatenate(np.transpose(X_val, (0, 2, 1, 3)), axis=0)
        X_test = np.reshape(X_test, (-1, X_test.shape[2], X_test.shape[3], X_test.shape[4]))
        X_test = np.concatenate(np.transpose(X_test, (0, 2, 1, 3)), axis=0)
